...
class DIContainerTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testContainerResolvesClassWithoutDependencies()
    {
        ...
        $this->assertTrue(
            $container->create("compot\\tests\\fixtures\\NoDependenciesClass") instanceof NoDependenciesClass
        );
    }
    ...
    public function testContainerWithUnBoundResolvableDependencies()
    {
        ...
        $container->bind($request);
        $container->bindTo(
            'compot\\tests\\fixtures\\DependencyInterface',
            DependencyBinder::to('compot\\tests\\fixtures\\NoDependenciesClass')
        );
    ...
    public function testContainerWithModelBinderResolvableDependencies()
    {
        ...
        $request   = Request::create(
            '/testUrl',
            'GET',
            array ("prop" => "propValue", "prop1" => "prop1Value", "prop2" => "prop2Value")
        );
        $container->setModelBinder(new DefaultModelBinder(new DefaultValueProvider($request)));
        $container->bindTo(
            'compot\\tests\\fixtures\\DependencyInterface',
            DependencyBinder::to('compot\\tests\\fixtures\\TestModelClass')
        );
        $container->bind($request);
